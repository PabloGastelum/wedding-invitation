...
-------------------------------------------*/
  ...
  if ($("#clock").length) {
    ...
    $("#clock").countdown(weddingDate, function (event) {
      var $this = $(this).html(
        event.strftime(
          "" +
            '<div class="box"><div><div class="time">%D</div> <span style="color: #848892">Días</span> </div></div>' +
            '<div class="box"><div><div class="time">%H</div> <span style="color: #848892">Horas</span> </div></div>' +
            '<div class="box"><div><div class="time">%M</div> <span style="color: #848892">Minutos</span> </div></div>' +
            '<div class="box"><div><div class="time">%S</div> <span style="color: #848892">Segundos</span> </div></div>'
        )
      );
    });
  }
  ...
  if ($("#contact-form-main").length) {
    $("#contact-form-main").validate({
      ...
      messages: {
        name: "Proporciona tu nombre",
        guest: "Seleccione el número de personas",
        contact: "Seleccione el contacto para confirmar",
      },
      submitHandler: function (form, event) {
        event.preventDefault();
        const serializeForm = $(form).serialize();
        const params = new URLSearchParams(serializeForm);
        ...
        const peopleText =
          guest === "uno"
            ? "una persona m%C3%A1s"
            : `${guest} personas m%C3%A1s`;

        const contactPhome = contact === "Pablo" ? "6441915208" : "6442611891";

        window.open(
          `https://api.whatsapp.com/send?phone=+52${contactPhome}&text=%2AHola%2C+${contact}+%F0%9F%91%8B+%2A%0AYo+*${name}*+confirmo+mi+asistencia+a+tu+boda+junto+a+${peopleText}.%0A%C2%A1Nos+vemos+pronto%21+%E2%98%BA%EF%B8%8F`
        );
